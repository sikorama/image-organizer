...
* `-p, --processedDir`: If set, original files are moved here after processing instead of staying in the source. *(Default: `null`)*
...
```bash
node index.js -o -m moondream -x -p ./backups
```
...
```bash
node index.js -s /mnt/old_drive -d /home/user/new_drive -k -o
```
...
```bash
node index.js -w -o -u http://192.168.1.1:11434 -m llava:13b
```
